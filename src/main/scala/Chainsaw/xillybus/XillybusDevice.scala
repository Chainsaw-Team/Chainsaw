 ...
 * @param deviceType = "fifo"/"mem" for stream(FIFO) or address based(RegFile or Mem) port
 * @param direction  = "write"/"read" for write (Host -> FPGA) or read (FPGA -> Host)
 * @param bitWidth   = 8/16/32, bit width of the fifo/register, can be
