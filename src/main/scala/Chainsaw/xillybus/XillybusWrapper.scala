...
case class XillybusWrapper(pinCount:Int, devices: Seq[XillybusDevice], target:ChainsawDevice) extends Component {
  ...
  assert(xillybus.memsBi.length + xillybus.memsRead.length + xillybus.memsWrite.length == 1, "only one mem device is supported")
  val innerMemBus = xillybus.memBiInterfaces.headOption
    .getOrElse(xillybus.memWriteInterfaces.headOption
  ...
  xillyDomain on {
    xillybus.streamsRead.zip(xillybus.streamReadInterfaces).zip(streamToHost).foreach { case ((device, busSide), userSide) =>
    ...
    xillybus.streamsWrite.zip(xillybus.streamWriteInterfaces).zip(streamFromHost).foreach { case ((device, busSide), userSide) =>
      // ignore open signal
