...
case class ChainsawDynamicInfiniteTest(
                                     testName: String = "testTemp",
                                     gen: ChainsawDynamicInfiniteGenerator,
  ...
  def getResetInterrupt = TestCase(Seq.fill(resetCycle)(randomInputVector).flatten, randomControl)
