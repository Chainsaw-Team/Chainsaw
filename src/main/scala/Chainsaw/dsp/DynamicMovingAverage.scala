...
case class DynamicMovingAverage(size: Int, dataType: NumericType)
  ...
  override def vivadoUtilEstimation = VivadoUtilEstimation(lut = (dataType.bitWidth * size * 1.2).toInt, dsp = 1)
  ...
  val latency = adderTreeLatency + scalingLatency + 4 // 4 for control compensations
  ...
  override def implH = new ChainsawDynamicInfiniteModule(this) {
    ...
    val sum: AFix = validElements.pipelinedBalancedTree(_ + _, 1).fixTo(dataType() << log2Up(size))

    // get mean
    val scalingFactorType = NumericType(integral = 0, fractional = 25, signed = true) // size suitable for both Xilinx and Intel DSP blocks
