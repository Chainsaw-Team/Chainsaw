...
class stateReg(G: Int, w: Int) extends Component {
  ...
  val writeArea = new Area {
    ...
    val regsNext = Vec(regs.zip(flipAvailAble.head.zip(flipAvailAble.last)).map { case (r, (f1, f2)) =>
      r ^ f1((1 << w) - 1 downto 1) ^ f2((1 << w) - 1 downto 1)
    })
...
class writeNetwork(G: Int, w: Int) extends Component {
  ...
  for (i <- addressSlice.indices) {
    dataArray(i + 1) =
      if (dataArray(i).getBitsWidth << addressSlice(i).getBitsWidth <= (1 << w))
        Vec(Reg(Bits(dataArray(i).getBitsWidth << addressSlice(i).getBitsWidth bits)) init 0)
      else
    ...
    if (dataArray(i + 1).length < (1 << addressSlice(i).getBitsWidth)) {
      ...
      for (j <- 0 until (1 << addressSlice(i).getBitsWidth)) {
        dataArray(i + 1)(j / length).subdivideIn(dataArray(i).head.getBitsWidth bits)(j % length) := Mux(
          addressSlice(i) === j,
          dataArray(i).head,
          B(0, dataArray(i).head.getBitsWidth bits)
        )
      }
    } else {
      for (j <- 0 until (1 << addressSlice(i).getBitsWidth)) {
        Vec(dataArray(i + 1).slice(j * dataArray(i).length, (j + 1) * dataArray(i).length)) := Mux(
          addressSlice(i) === j,
          dataArray(i),
          Vec(B(0, (1 << w) bits), dataArray(i).length)
        )
      }
    }
  }
...
class readNetwork(G: Int, w: Int) extends Component {
  ...
  for (i <- addressSlice.indices) {
    dataArray(i + 1) = Reg(
      if (dataArray(i).getBitsWidth >> addressSlice(i).getBitsWidth <= (1 << log2Up(G)))
        Vec(Bits(1 bits), dataArray(i).getBitsWidth >> addressSlice(i).getBitsWidth)
      else
    ...
    for (j <- dataArray(i + 1).indices) {
      dataArray(i + 1)(j) := Vec(dataArray(i).slice(j * length, (j + 1) * length))(
        addressSlice(i)(addressSlice(i).getBitsWidth - 1 downto addressSlice(i).getBitsWidth - log2Up(length))
      ).subdivideIn(dataArray(i + 1)(j).getBitsWidth slices)
        .map(_(addressSlice(i)(addressSlice(i).getBitsWidth - log2Up(length) - 1 downto 0)).asBits)
        .reduce(_ ## _)
        .reversed
    }
  }
