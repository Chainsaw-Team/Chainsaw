...
class Retiming() extends Phase {
  override def impl(pc: PhaseContext): Unit = {
    ...
    require(
      GlobalData.get.toplevel.isInstanceOf[ChainsawBaseModule],
      "for retiming, the top level module must be a ChainsawBaseModule"
    )
...
class RetimingGraph extends ExpressionGraph {
  ...
  /** solve retiming by linear programming on a component graph
    *
    * @return
    *   a map from leave components to its retiming value
    */
  ...
  def doRetiming(top: ChainsawBaseModule): Unit = {
    ...
    var paddingCount = 0
    vertexSet().asScala
      .map(_.asInstanceOf[BaseType]) // find padding value vertex by vertex
      .foreach { v =>
        ...
        require(
          paddings.forall(padding => padding == paddings.head && padding >= 0),
          s"padding value of $v is not the same: ${paddings.mkString(" ")}"
        )
      ...
      current.children.foreach { c =>
        ...
        if (!child.gen.isInstanceOf[OverwriteLatency]) leaves += child
          ...
          if (child.children.isEmpty)
            throw new IllegalArgumentException(s"component $child is a leaf, but has no fixed latency")
          else dfs(child)
        }
      }
    }
