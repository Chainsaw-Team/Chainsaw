...
object Gpcs {
  def apply(): Seq[Compressor] = {
    ...
    //    val ret = Seq(Compressor1to1, Compressor6to3, Compressor3to2)
    ret
  }
}
...
object Compressor4to2 extends Compressor {
  ...
  override def clbCost(width: Int, considerCarry8: Boolean = true, isPipeline: Boolean = true): Double =
    Seq(
      vivadoUtilEstimation(width).lut.toDouble,
      if (considerCarry8) vivadoUtilEstimation(width).carry8 * 8.0 else 0.0,
      if (isPipeline) vivadoUtilEstimation(width).ff.toDouble / 2 else 0.0
    ).max
  ...
  override def vivadoUtilEstimation(width: Int) =
...
object Compressor1to1 extends Compressor {
  ...
  override def vivadoUtilEstimation(width: Int) = VivadoUtilRequirement(ff = width)
...
object Compressor6to3 extends Compressor {
  ...
  override def clbCost(width: Int, considerCarry8: Boolean = true, isPipeline: Boolean = true): Double = 3.0
...
object Compressor3to2 extends Compressor {
  ...
  override def clbCost(width: Int, considerCarry8: Boolean = true, isPipeline: Boolean = true): Double = 1.0
