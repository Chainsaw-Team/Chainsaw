...
import Chainsaw.deprecated.BitHeaps._
import org.json4s._
...
case class BitHeaps[T](bitHeapConfigInfo: BitHeapConfigInfo[T]*) {
  ...
  def finalStage = currentTime == times.max && currentHeights.count(_ > 3) < currentWidth / 2
  ...
  def getExactReductionEfficiency(
    ...
    (bits - // bitsIn
      ...
      compressor.clbCost(width, considerCarry8, isPipeline) // divided by cost
  }
  ...
  def compressOneTime(
    ...
    val areaCost = bestCompressor.clbCost(bestWidth, considerCarry8, shouldPipeline)
  ...
  def compressOneStage(
    ...
    if (nextStage.height <= finalHeight && nextStage.bitHeaps.length <= 1 && !shouldPipeline) {
      ...
      stageAreaCost = compressorSolutions.map(solution => compressors.find(_.name == solution.compressorName).get.clbCost(solution.width, considerCarry8, shouldPipeline)).sum
    }
