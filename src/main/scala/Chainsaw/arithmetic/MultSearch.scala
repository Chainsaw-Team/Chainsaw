...
object MultSearch {
  ...
  def apply(width: Int, multiplierType: MultiplierType, budget: VivadoUtil): Unit = {
    ...
    val count = budget.solveBestScheme(solutions.map(_.vivadoUtilEstimation + VivadoUtilRequirement(lut = 20000, dsp = 0)), Seq(0, 2)) // 0 & 2 for LUT & DSP
    count.zip(solutions).foreach { case (i, solution) => logger.info(s"$i X $solution") }
    val util = count.zip(solutions).map { case (i, solution) => solution.vivadoUtilEstimation * i }.reduce(_ + _)
  ...
  def getBmParetos(width: Int, multiplierType: MultiplierType, constant: Option[BigInt] = None): Seq[MultAttribute] = {
    ...
    def inferior(a: MultAttribute, b: MultAttribute): Boolean = // definition of Pareto
      a.clbCost >= b.clbCost && a.dspCost >= b.dspCost

    def equal(a: MultAttribute, b: MultAttribute) = a.clbCost == b.clbCost && a.dspCost == b.dspCost

    def makePareto(solutions: ArrayBuffer[MultAttribute]): mutable.Seq[MultAttribute] = {
      ...
      Seq.tabulate(n, n) { (i, j) =>
        if (i != j) if (inferior(solutions(i), solutions(j))) {
          if (equal(solutions(i), solutions(j))) inferiors += solutions(i max j) // always drop the smaller one
    ...
    if (constant.isDefined) {
      if (!solutionSet.isDefinedAt(width)) solutionSet(width) = ArrayBuffer[MultAttribute]()
      multiplierType match {
        case FullMultiplier => solutionSet(width) += FullBcm(constant.get, width)
        case MsbMultiplier => solutionSet(width) += MsbBcm(constant.get, width, width, width)
        case LsbMultiplier => solutionSet(width) += LsbBcm(constant.get, width, width)
      }
    }
    ...
    def addSolution(solutionNext: BmSolution): Unit = {
      if (!inferior(solutionNext, naiveSolution)) {
        ...
        if (!solutionSet.isDefinedAt(widthFull)) solutionSet(widthFull) = ArrayBuffer[MultAttribute]()
    ...
    logger.info(s"Pareto solutions for $width-bit ${if (constant.isDefined) "constant" else "variable"} ${className(multiplierType)}:" +
      s"\n${
        ret.map {
