...
import Chainsaw.xilinx.{VivadoRequirement, VivadoUtil}
import spinal.lib._
...
case class DSPBlock (
  ...
  override val params = {
    if(usePostAdder)
      Seq(
        ("wX", wX), ("wY", wY), ("wZ", wZ), ("xIsSigned", if(xIsSigned) 1 else 0),
        ("yIsSigned", if(yIsSigned) 1 else 0), ("isPipelined", if(isPipelined) 1 else 0),
        ("usePostAdder", 1), ("usePreAdder", if(usePreAdder) 1 else 0),
        ("preAdderSubtracts", if(preAdderSubtracts) 1 else 0)
      )
    else
      Seq(
        ("wX", wX), ("wY", wY), ("xIsSigned", if(xIsSigned) 1 else 0),
        ("yIsSigned", if(yIsSigned) 1 else 0), ("isPipelined", if(isPipelined) 1 else 0),
        ("usePreAdder", if(usePreAdder) 1 else 0), ("preAdderSubtracts", if(preAdderSubtracts) 1 else 0)
      )
  }
  ...
  override def inputTypes: Seq[NumericType] = {
    if(usePreAdder && usePostAdder){
      Seq(
        NumericType(wX - (if(xIsSigned) 1 else 0), 0, signed = xIsSigned), NumericType(wX - (if(xIsSigned) 1 else 0), 0, signed = xIsSigned),
        NumericType(wY - (if(yIsSigned) 1 else 0), 0, signed = yIsSigned), NumericType(wZ - (if(xIsSigned) 1 else 0), 0, signed = xIsSigned)
      )
    }
    ...
    def getVector: Seq[BigDecimal] = {
      ...
      def satisfied: Boolean = {
        ...
        val resultNotOverflow = {
          if(usePreAdder) inRange(widthOut, xIsSigned, (if(preAdderSubtracts) (X1-X2)*Y else (X1+X2)*Y) + (if(usePostAdder) Z else 0.toBigInt))
