...
abstract class FastAdditionAlgo extends HardAlgo {
  ...
  def selfTest(): Unit = Seq.fill(1000)(Seq.fill(2)(BigInt(width, Random)) :+ BigInt(1, Random))
    .foreach { case Seq(a, b, cin) => assert(impl(a, b, cin) == (a + b + cin), s"$a + $b + $cin != ${impl(a, b, cin)}") }
}
...
abstract class CarrySelectAddition extends FastAdditionAlgo {
  ...
  def getWords(a: BigInt, b: BigInt) = {
    val aWords = a.toBitValue(width).splitN(blockCount)
    //    println("aWords: " + aWords.mkString(" "))
    val bWords = b.toBitValue(width).splitN(blockCount)
    //    println("bWords: " + bWords.mkString(" "))
    (aWords, bWords)
  }

  def combineWords(sWords: Seq[BigInt], carryOut: BigInt): BigInt =
    (carryOut << (blockWidth * blockCount)) + sWords.zipWithIndex.map { case (word, index) => word << (blockWidth * index) }.sum
...
case class AamAddition(width: Int, blockWidth: Int) extends CarrySelectAddition {
  ...
  override def impl(a: BigInt, b: BigInt, cin: BigInt) = {
    ...
    val c0AndS0s = aWords.zip(bWords).map { case (a, b) => a + b }.map(_.toBitValue(blockWidth + 1).splitAt(blockWidth))
    val c1AndS1s = aWords.zip(bWords).map { case (a, b) => a + b + 1 }.map(_.toBitValue(blockWidth + 1).splitAt(blockWidth))
    ...
    // stage 3, MUXes
    val sWords = s0s.zip(s1s).zip(cs).map { case ((s0, s1), c) => if (c == 1) s1 else s0 }
    combineWords(sWords, cs.last)
  }

  override def vivadoUtilEstimation: VivadoUtil = VivadoUtilEstimation(lut = (3 * blockCount + 1) * blockWidth)
}
...
case class CaiAddition(width: Int, blockWidth: Int) extends CarrySelectAddition {
  ...
  override def impl(a: BigInt, b: BigInt, cin: BigInt) = {
    ...
    val c0s = c0AndS0s.map(_._1) // low to high
    val s0s = c0AndS0s.map(_._2)
    val c1s = aWords.zip(bWords).map { case (a, b) => a >= ~b.toBitValue(blockWidth) }.map(if (_) BigInt(1) else BigInt(0))
    // stage 2, CCC
    val cs = ccc(cin, c0s, c1s)
    // stage 3, RCAs
...
class CcaAddition(val width: Int, val blockWidth: Int) extends CarrySelectAddition {
  ...
  override def impl(a: BigInt, b: BigInt, cin: BigInt) = {
    ...
    val c0s = aWords.zip(bWords).map { case (a, b) => a > ~b.toBitValue(blockWidth) }.map(if (_) BigInt(1) else BigInt(0))
    ...
    // stage 3, RCAs
    val sWords = aWords.zip(bWords).zip(cs).map { case ((a, b), c) => (a + b + c).mod(Pow2(blockWidth)) }
    println("sWords: " + sWords.mkString(" "))
    combineWords(sWords, cs.last)
  }

  // when stage 1 is pipelined, stage 2 & 3 are combined and pipelined
  override def vivadoUtilEstimation: VivadoUtil = VivadoUtilEstimation(lut = 2 * width + blockCount, ff = 4 * width)
}

object CcaAddition {
  def apply(width: Int, blockWidth: Int): CcaAddition = new CcaAddition(width, blockWidth)
