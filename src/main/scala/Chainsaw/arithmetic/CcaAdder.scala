...
/** long fast adder based on carry selection/increment, this consumes twice as much LUTs as binary CPA, assuring a O(\sqrt{w}) critical path and constant latency = 2
 * @param width width of the binary adder
 * @param blockWidth width of a block
 * @see ''Nguyen, Hong Diep, Bogdan Mihai Pasca and Thomas B. Preußer. “FPGA-Specific Arithmetic Optimizations of Short-Latency Adders.” 2011 21st International Conference on Field Programmable Logic and Applications (2011): 232-237.''
 */
case class CcaAdder(override val width: Int, override val blockWidth: Int) extends
  CcaAddition(width: Int, blockWidth: Int) with ChainsawOperatorGenerator {
  ...
  override def inputTypes = Seq.fill(2)(NumericType.U(width)) :+ NumericType.U(1)

  override def outputTypes = Seq(NumericType.U(width + 1))

  override def impl(testCase: TestCase) = Seq(BigDecimal(testCase.data.map(_.toBigInt()).sum))
  ...
  override def implH = new ChainsawOperatorModule(this) {
    // TODO: first block needs no selection
    ...
    val (c0temp, sumWord0temp) = (aHead.head +^ bHead.head + cin).splitAt(blockWidth)
    ...
    val cs = c01 +: carryChains.map(_.CO).reverse.reduce(_ @@ _).asBools.take(width).map(_.asUInt)
    val cout = cs(blockCount - 1)

    // 2 -> 3, RCAs, LUT = width - blockWidth, FF = width * 2 + 1
    val sWords = aWords1.zip(bWords1).zip(cs).map { case ((a, b), c) => (a + b + c) }
    dataOut.head := (cout.d() @@ (sWord0 +: sWords).map(_.d()).reverse.reduce(_ @@ _)).toAFix
  }

  override def vivadoUtilEstimation = VivadoUtilEstimation(lut = width * 2 + blockCount - blockWidth, ff = width * 4 - blockWidth + blockCount)
}
