...
class BmAlgo(val bmSolution: BmSolution)
  ...
  def merge(weightedValues: Seq[WeightedBigInt], widthOut: Int): WeightedBigInt = {
        ...
        currentType match {
          case FullMultiplier =>
            if (isKara) {
              ...
              val prods: Seq[WeightedBigInt] = Seq.tabulate(split, split) { (i, j) =>
