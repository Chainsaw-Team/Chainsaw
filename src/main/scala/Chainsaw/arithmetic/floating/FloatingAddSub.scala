...
case class FloatingAdd(exponentSize: Int, mantissaSize: Int, family: XilinxDeviceFamily, targetFrequency: HertzNumber)
    extends Module {
...
case class FloatingSub(exponentSize: Int, mantissaSize: Int, family: XilinxDeviceFamily, targetFrequency: HertzNumber)
    extends Module {
  val x, y = in(Floating(exponentSize, mantissaSize))
  val z    = out(Floating(exponentSize, mantissaSize))
  ...
  def main(args: Array[String]): Unit = {
    ...
    SimConfig.withFstWave.compile(SinglePrecisionFPAdd(UltraScale, 20 MHz)).doSim { dut =>
      ...
      for (_ <- 0 until 10000) {
        ...
        if (!(((expected - dut.z.toFloat).abs < 1e-37) || (expected.isInfinite && dut.z.isNaN)))
      ...
      for (_ <- 0 until 10000) {
        ...
        dut.y.randDenormal
        dut.clockDomain.waitSampling(2)
        if (!((dut.z.toFloat == dut.y.toFloat) || (dut.z.toFloat == 0)))
          simFailure(s"FPAdd error, z=${dut.z.toFloat}, y=${dut.y.toFloat}")
      }
    ...
    SimConfig.withFstWave.compile(DoublePrecisionFPAdd(UltraScale, 20 MHz)).doSim { dut =>
      ...
      for (_ <- 0 until 10000) {
        ...
        dut.clockDomain.waitSampling()
        if (!(((expected - dut.z.toDouble).abs < 1e-37) || (expected.isInfinite && dut.z.isNaN)))
      ...
      for (_ <- 0 until 1000) {
        ...
        dut.y.randDenormal
        dut.clockDomain.waitSampling(2)
        if (!((dut.z.toDouble == dut.y.toDouble) || (dut.z.toDouble == 0)))
          simFailure(s"FPAdd error, z=${dut.z.toFloat}")
      }
    ...
    SimConfig.withFstWave.compile(SinglePrecisionFPSub(UltraScale, 20 MHz)).doSim { dut =>
      ...
      for (_ <- 0 until 1000) {
        ...
        dut.y #= 0.0f
        dut.clockDomain.waitSampling(2)
        if (!((dut.z.toFloat == dut.x.toFloat) || (dut.z.toFloat == 0)))
          simFailure(s"FPAdd error, z=${dut.z.toFloat}, y=${dut.y.toFloat}")
      }
    ...
    SimConfig.withFstWave.compile(DoublePrecisionFPSub(UltraScale, 20 MHz)).doSim { dut =>
      ...
      for (_ <- 0 until 10000) {
        ...
        dut.clockDomain.waitSampling()
        if (!(((expected - dut.z.toDouble).abs < 1e-37) || (expected.isInfinite && dut.z.isNaN)))
          simFailure(s"FPAdd error, z=${dut.z.toDouble}, expected=$expected")
      }
      ...
      for (_ <- 0 until 1000) {
        ...
        dut.y #= 0.0
        dut.clockDomain.waitSampling(2)
        if (!((dut.z.toDouble == dut.x.toDouble) || (dut.z.toDouble == 0)))
          simFailure(s"FPAdd error, z=${dut.z.toFloat}")
      }
