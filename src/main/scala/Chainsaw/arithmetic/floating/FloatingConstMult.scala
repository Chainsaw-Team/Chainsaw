...
) extends BlackBox with floatingFlopoco {
  override val operatorName = "FPConstMult"
  override val entityName = operatorName
  override val params = Seq(
    ("wE_in", exponentSizeIn), ("wF_in", mantissaSizeIn),
    ("wE_out", exponentSizeOut), ("wF_out", mantissaSizeOut),
    ("constant", "\"" + constant + "\"")
  )
...
case class FloatingConstMult (
  ...
  val constMult = FlopocoConstMult(exponentSizeIn, mantissaSizeIn, exponentSizeOut, mantissaSizeOut, constant, family, targetFrequency)

  i2f.X := x.asBits
  constMult.X := i2f.R(i2f.R.high-1, 2 bits) ## i2f.R(0, (exponentSizeIn+mantissaSizeIn) bits)
  f2i.X := constMult.R(constMult.R.high-1, 2 bits) ## False.asBits ## constMult.R(0, (exponentSizeOut+mantissaSizeOut) bits)
  z.assignFromBits(RegNext(f2i.R)) // latency = 1
}

object SinglePrecisionFPConstMult {
  def apply(constant: Double, family: XilinxDeviceFamily, targetFrequency: HertzNumber) = FloatingConstMult(8, 23, 8, 23, constant, family, targetFrequency)
}

object DoublePrecisionFPConstMult {
  def apply(constant: Double, family: XilinxDeviceFamily, targetFrequency: HertzNumber) = FloatingConstMult(11, 52, 11, 52, constant, family, targetFrequency)
}
...
object FloatingConstMultSim {
  def main(args: Array[String]): Unit = {
    SimConfig.withFstWave.compile(SinglePrecisionFPConstMult(Math.PI, UltraScale, 200 MHz)).doSim{dut=>
      ...
      dut.clockDomain.waitSampling(2)
      if(!((dut.z.mantissa.toInt==0) && (dut.z.exponent.toInt==0))) simFailure(s"FPConstMult error, z=${dut.z.toFloat}")
...
//      simFailure("error")
    ...
    SimConfig.withFstWave.compile(DoublePrecisionFPConstMult(Math.PI, UltraScale, 200 MHz)).doSim{dut=>
      ...
      dut.clockDomain.waitSampling(2)
      if(!((dut.z.mantissa.toBigInt==0) && (dut.z.exponent.toInt==0))) simFailure(s"FPConstMult error, z=${dut.z.toDouble}")
