...
case class BitHeapConfigInfo[T](bitHeap: ArrayBuffer[ArrayBuffer[T]], weightLow: Int, time: Int) {
  def +(that: BitHeapConfigInfo[T]): BitHeapConfigInfo[T] = {
    require(time == that.time, "add two BitHeap ConigInfo must have same time value !")
...
case class BitHeap[T](bitHeapConfigInfo: BitHeapConfigInfo[T]*) {
  // TODO +s
  val newBitHeapConfigInfo: Seq[BitHeapConfigInfo[T]] = bitHeapConfigInfo.groupBy(_.time).toSeq.map { case (_, infos) => infos.reduce(_ + _) }
  ...
  def d(pipeline: T => T): BitHeap[T] = {
    val pipelinedHeap = BitHeap(bitHeaps.zip(weightLows.zip(times)).map { case (bitHeap, (weightLow, time)) => BitHeapConfigInfo(bitHeap.map(_.map(pipeline)), weightLow, time) }: _*) // pipeline the whole bit heap
  ...
  def implCompressOneStage(compressors: Seq[Compressor], stageSolution: StageSolution, pipeline: Bool => Bool): BitHeap[Bool] = {
    ...
    val nextStageHeap = this.asInstanceOf[BitHeap[Bool]] + partialNextStage
  ...
  def headStrategy(
    ...
    if (currentHeights(columnIndex) == 2 && columnIndex <= currentWidth - 2) {
      abnormalCompress = (currentHeights(columnIndex) + carryRecords.getOrElse(columnIndex, 0) + nextHeapCurrentHeight >= 3 &&
        currentHeights(columnIndex) + carryRecords.getOrElse(columnIndex, 0) + nextHeapCurrentHeight <= currentHeights(columnIndex + 1) + carryRecords.getOrElse(columnIndex + 1, 0) + nextHeapNextHeight) ||
  ...
  def tailStrategy(
    ...
    val formalCompress =
      if (columnIndex <= currentWidth - 2) {
        if (!isPipeline) currentHeights(columnIndex) == 2 && carryRecords.getOrElse(columnIndex, 0) == 0 && currentHeights(columnIndex + 1) + carryRecords.getOrElse(columnIndex + 1, 0) == 2 || currentHeights(columnIndex) + carryRecords.getOrElse(columnIndex, 0) > 3
  ...
  def compressOneTime(
    ...
    val (bestCompressor, bestWidth, columnIndex, bestReductionEfficiency) = if (finalStageBefore) tailStrategy(compressors, considerCarry8, tailBound, useHeightControlStrategy) else headStrategy(compressors, considerCarry8, headBound, useHeightControlStrategy)
  ...
  def compressOneStage(
    ...
    while (!currentIsEmpty) {
      val (heap, compressorSolution) = {
        compressOneTime(compressors, considerCarry8, headBound, tailBound, finalStageBefore, shouldPipeline)
      }
      results += heap
  ...
  def compressAll(
    ...
    val solutionsFile = new File(compressorSolutionDir, s"${this.toString.hashCode()}")
    if (solutionsFile.exists() && useHistorySolutions) {
      ...
      while ((current.height > 2 || current.bitHeaps.length > 1) && latency < 100) {
        ...
        bitRatioTarget = if (currentBefore.finalStage) bitRatioTargetList.last else bitRatioTargetList.head
        reductionEfficiencyTarget = if (currentBefore.finalStage) reductionEfficiencyTargetList.last else reductionEfficiencyTargetList.head
        ...
        if (stageSolution.getReductionRatio >= bitRatioTarget && stageSolution.getReductionEfficiency >= reductionEfficiencyTarget) {
          ...
          if (headEfficiencyBound > 0.2 && tailEfficiencyBound > 0.2) {
            if (currentBefore.finalStage) tailEfficiencyBound -= 0.1 else headEfficiencyBound -= 0.1
            current = currentBefore
          } else {
            headEfficiencyBound = initHeadEfficiencyBound
            tailEfficiencyBound = initTailEfficiencyBound
            val (finalStageSolution, (finalHeap, finalStageLog)) = candidateStageSolutions.zip(candidateBitHeaps.zip(candidateStageLogs)).sortBy(_._1.getReductionEfficiency).reverse.head
            ...
            if (verbose >= 1) logger.info(s"optimal headBound: ${candidateHeadBounds(candidateStageSolutions.indexOf(finalStageSolution))}\toptimal tailBound: ${candidateTailBounds(candidateStageSolutions.indexOf(finalStageSolution))}")
      ...
      logger.info(
        s"\n----efficiency report of bit heap compressor----" +
          ...
          s"\n\tcompressor1to1 number percent: ${stageSolutions.flatMap(_.compressorSolutions.map(_.compressorName)).count(_ == "Compressor1to1").toDouble / stageSolutions.map(_.compressorSolutions.length).sum}" +
          s"\n\tcompressor1to1 final number percent: ${stageSolutions.filter(_.isFinalStage).flatMap(_.compressorSolutions.map(_.compressorName)).count(_ == "Compressor1to1").toDouble / stageSolutions.map(_.compressorSolutions.length).sum}" +
          s"\n\tcompressor1to1 cost percent: ${stageSolutions.flatMap(_.compressorSolutions).filter(_.compressorName == "Compressor1to1").map(_.getAreaCost).sum / stageSolutions.flatMap(_.compressorSolutions).map(_.getAreaCost).sum}" +
          s"\n\tcompressor1to1 final cost percent: ${stageSolutions.filter(_.isFinalStage).flatMap(_.compressorSolutions).filter(_.compressorName == "Compressor1to1").map(_.getAreaCost).sum / stageSolutions.flatMap(_.compressorSolutions).map(_.getAreaCost).sum}" +
          s"\n\tcompressor1to1 number : all compressor number: (${stageSolutions.flatMap(_.compressorSolutions.map(_.compressorName)).count(_ == "Compressor1to1")}, ${stageSolutions.map(_.compressorSolutions.length).sum})" +
          s"\n\tcompressor1to1 final number : all compressor number: (${stageSolutions.filter(_.isFinalStage).flatMap(_.compressorSolutions.map(_.compressorName)).count(_ == "Compressor1to1")}, ${stageSolutions.map(_.compressorSolutions.length).sum})" +
          s"\n\tcompressor1to1 cost : all cost: (${stageSolutions.flatMap(_.compressorSolutions).filter(_.compressorName == "Compressor1to1").map(_.getAreaCost).sum}, ${stageSolutions.flatMap(_.compressorSolutions).map(_.getAreaCost).sum})" +
          s"\n\tcompressor1to1 final cost : all cost: (${stageSolutions.filter(_.isFinalStage).flatMap(_.compressorSolutions).filter(_.compressorName == "Compressor1to1").map(_.getAreaCost).sum}, ${stageSolutions.flatMap(_.compressorSolutions).map(_.getAreaCost).sum})"
      )
      ...
      logger.info(s"Store a solution to path : $compressorSolutionDir/${this.toString.hashCode()}")
  ...
  // deep copy
  def copy: BitHeap[T] = {
    val copyRet = BitHeap(this.bitHeaps.map(_.map(_.map(i => i))), this.weightLows, this.times)
...
object BitHeap {
  ...
  def getInfosFromBitHeap[T](bitHeap: BitHeap[T]): Seq[Seq[ArithInfo]] = {
