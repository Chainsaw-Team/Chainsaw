 ...
 *
 */
case class LUT5to2(expressionO5: (Boolean, Boolean, Boolean, Boolean, Boolean) => Boolean,
  ...
  val init = (LUT6_2.expression2value(exp1, 5) << 32) + LUT6_2.expression2value(exp0, 5)
  ...
  override def vivadoUtilEstimation = VivadoUtilEstimation(lut = 1, ff = 0)
...
object LUT6_2 {
  ...
  def getExpressionWithInverse(expression: Seq[Boolean] => Boolean, inverseList: Seq[Boolean]): Seq[Boolean] => Boolean = {
    data => {
      val inversed = data.zip(inverseList).map { case (bool, inverse) => if (inverse) !bool else bool }
      expression(inversed)
    }
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(expression2value(getExpressionWithInverse(exp1, Seq(false, false, true, false, false)), 5).toString(16) + expression2value(getExpressionWithInverse(exp0, Seq(false, false, true, false, false)), 5).toString(16))
    println(expression2value(getExpressionWithInverse(exp1, Seq(false, true, true, false, false)), 5).toString(16) + expression2value(getExpressionWithInverse(exp0, Seq(false, true, true, false, false)), 5).toString(16))
  }
}