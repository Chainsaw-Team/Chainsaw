...
import Chainsaw.device.MULTMODE
 ...
 * @param InnerDelayAB  1 for A1B1, 2 for A2B2
...
class DSP48E2Cell_MultAdd(nameIdx: Int, InnerDelayAB:Int = 1, ExtDelayAB: Int = 0, ExtDelayC: Int = 1) extends Component {
  ...
  val dsp = new DSP48E2(attr).setName(s"dsp_MultAdd_${nameIdx}")
...
object DSP48E2Cell_MultAdd_Main {
  def main(args: Array[String]) {
    //NewSpinalConfig.generateVerilog(new DSP48E2Cell_MultAdd(0, 1, 0,1)).printRtl()
    NewSpinalConfig.generateVerilog(new DSP48E2Cell_MultAdd(0, 2, 0,2)).printRtl()
  }
}
...
class DSP48E2Cell_AddMultMinus(nameIdx: Int, ExtDelayABD: Int = 0, ExtDelayC: Int = 1) extends Component {
  ...
  val dsp = new DSP48E2(attr).setName(s"dsp_AddMultMinus_${nameIdx}")
  dsp.RSTs.all.foreach(_.clear())
  dsp.CEs.all.foreach(_.set())
...
object DSP48E2Cell_AddMultAdd_Main {
  def main(args: Array[String]) {
    NewSpinalConfig.generateVerilog(new DSP48E2Cell_AddMultMinus(0,0,2)).printRtl()
  }
}
