...
case class ChipVerify() extends AXKU041 {
  ...
  val dataArea =
    new ClockingArea(dmaClockDomain) {
      ...
      pulseGen.triggerMode         := getControlWordCC(ctrlArea.triggerMode)
      pulseGen.voltageForSample    := getControlWordCC(ctrlArea.voltageForSample)
      pulseGen.voltage1ForCamera   := getControlWordCC(ctrlArea.voltageForCamera)
      pulseGen.pulseWidthForSample := getControlWordCC(ctrlArea.pulseWidthForSample)
      pulseGen.pulseWidthForCamera := getControlWordCC(ctrlArea.pulseWidthForCamera)
      pulseGen.delay0              := getControlWordCC(ctrlArea.delay0)
      pulseGen.delay1              := getControlWordCC(ctrlArea.delay1)
      pulseGen.delay2              := getControlWordCC(ctrlArea.delay2)
      // pulseGen -> DA(AN9767)
      ...
      imageRx.streamOut.queue(16, dmaClockDomain, xillybus.xillyDomain) >> upload_32
