...
class DeviceIpTests extends ChainsawFlatSpec {
  ...
  it should "get inversed" in {
    ...
    (0 until 100).foreach { _ =>
      ...
      val boolsMasked = bools.zip(inverseMask).map { case (i, bool) => if (bool) 1 - i else i }
      val addrMasked = BigInt(boolsMasked.reverse.mkString(""), 2)
      assert(valueAfter.toString(2).reverse.padTo(64, '0').apply(addr.toInt) == valueBefore.toString(2).reverse.padTo(64, '0').apply(addrMasked.toInt),
